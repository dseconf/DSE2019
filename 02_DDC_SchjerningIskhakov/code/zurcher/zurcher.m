classdef zurcher
    ...
    methods (Static)
        ...
        function [pp, pp_K, pp_R] = eqb(mp,P, pk)
            ...
            % transition matrix + choice probs
            tmp=bsxfun(@times, P{1}(:,2:mp.n), pk(2:mp.n,1)');
            pl=([1-(sum(tmp,2)) tmp]);

            pp = ergodic(pl);

            % OLD CODE REPLACED WITH ergodic()
            % y=0.99999999;
            % pp=inv((eye(mp.n)-y*pl)')*((1-y)*ones(mp.n,1)/mp.n);
            % pp=pp'/sum(pp);
            % for i=1:10000    ;
            %   pp=pp*pl;   
            %   tol=max(abs(pp-pp*pl)');
            %   if tol<0.000000001; 
            %     fprintf('zurcher.eqb, converged in %d iterations, tol=%g\n', i, tol);
            %     break;
            %   end
            % end;

            pp_K=pp;  
            pp_K(1,1)=mp.p(1)*pp(1,1)*pk(1,1);
            pp_R=pp.*(1-pk)'; 
